...
`PromoteProspectModal.test.tsx`'s existing `hasAttribute("defaultValue")`
assertion is worth flagging as **vacuous**: React never emits a literal
`defaultValue` attribute, so it reads `false` for controlled and uncontrolled
inputs alike and cannot fail. It was left in place — it is harmless and matches
the wording in CLAUDE.md — but it is not what proves anything. `form.reset()` is.
