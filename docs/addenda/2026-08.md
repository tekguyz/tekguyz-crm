...
- **Pills desaturated** by chroma × 0.78 (~22%) with lightness untouched, so fg/bg contrast is preserved and only saturation drops. DESIGN.md v2 specified the cut for light mode only; the same ratio was applied to dark so the two themes read as one palette, and that decision is now recorded in `DESIGN.md`.
...
**Files changed (5).** `src/app/globals.css` (`--color-cold-fg` in `@theme inline`, plus the token in both theme blocks with the rationale comment), `src/components/ui/Badge.tsx`, `src/components/ui/Badge.test.tsx` (the cold-tone test now asserts `text-cold-fg` **and** `not.toHaveClass("text-cold")`, so a revert fails loudly), `DESIGN.md` (token table row, and `--cold`'s Use column narrowed to "border only"), `docs/KNOWN_GAPS.md` (bullet relocated to the Resolved Items Archive above). No new `text-*` type role, so `src/lib/utils/cn.ts` needs no change — `text-cold-fg` is a colour, which tailwind-merge already classifies correctly. `npx vitest run src/components/ui/Badge.test.tsx`: 6 passed. No migration, no database write, no test residue.
...
written into `DESIGN.md` § The Application Shell — they belong to the
...
- a **value that already lives in `src/app/globals.css` or `DESIGN.md`**
...
stayed in CLAUDE.md rather than moving to `DESIGN.md` deliberately:
...
`prefers-color-scheme`, not the `.dark` class), and `DESIGN.md` was added to
...
table. It rescued two. An audit of CLAUDE.md, `DESIGN.md` and
...
`DESIGN.md` § The Application Shell. Only that clause and its reason came
...
four docs *plus* a scripted `check-design-drift.mjs` comparing `DESIGN.md`'s
...
is a build-time script, and `DESIGN.md` already documents the package as a
`-D` install.
...
`docs/WEBHOOK_INTEGRATION.md` (**since removed, 2026-08-19** — see the note at the end of this section). Deleted:
...
repo's own root.** The handoff spec that draft produced has been deleted. `docs/WEBHOOK_INTEGRATION.md` was written as the protocol doc for any
external integrator, but **was itself removed on 2026-08-19** as not belonging
in this repo: the only real integrator is the sibling site checkout, and a spec
doc with one in-house consumer is a second copy that drifts from
`src/lib/webhooks/signature.ts` rather than a contract anyone outside reads.
That file and its test are now authoritative for the header spelling, the
digest and the raw-body rule.
...
## 2026-08-19 — Reporting view: `/reports`

**What shipped.** A read-only, all-time aggregate view at `/reports`, plus
`src/lib/leads/report-queries.ts` (`getPipelineReport`) and three presentation
files under `src/components/reports/`. One nav entry, first in `SECONDARY_NAV`,
ahead of Import and Settings — the mobile tab bar's four slots were not
reopened, so it appears only in the "More" sheet on a phone and in the single
flat sidebar list on desktop. **No schema change, no migration, no RLS work, no
new grant**: every figure comes from `leads` columns that were already live and
already readable by every role in the tenant.

**Why it exists.** `status`, `outcome`, `actual_revenue`, `closed_at` and
`estimated_revenue` were all populated and had exactly one consumer — the weekly
cron email. Nothing in the product showed any of it.

**Aggregation runs in JS, not SQL, and that is a deliberate scope decision.**
PostgREST has no `GROUP BY`, and both alternatives — a `SECURITY DEFINER` RPC or
a materialised view — are schema work. Two SELECTs of two columns each, summed
in the request, is cheaper than either at the row counts `docs/KNOWN_GAPS.md`
records. Revisit only if a real tenant's `leads` table stops fitting in one
response.

**The two halves filter differently, matching `aggregate-org-revenue.ts`.**
Open pipeline is `outcome IS NULL AND archived = false` — an archived lead is
one a human removed, so it is not still in play. Closed leads are
`outcome IS NOT NULL` with **no** `archived` filter — a won deal stays won after
someone tidies it out of the directory, and filtering it would silently shrink
realized revenue. Both are stated in the UI next to the numbers they qualify,
because a total whose scope is invisible is a total nobody can check.

**Win rate is `WON / (WON + LOST)`, and the formula is printed on the page.**
`ABANDONED` is excluded from the denominator: an abandoned lead never reached a
decision, so counting it as a loss would punish a team for a prospect that went
quiet. That is a judgement call, not an obvious one, so the page says so in
words rather than leaving the reader to infer it from a percentage. When nothing
has been decided the figure renders as `—` with "No leads decided yet", never as
`0%` — null is not zero.

**Colour.** The only colour the view spends is the existing stage language: each
stage row carries its pill Badge and a 4px share bar in the matching
`--pill-*-fg`, the same mapping `STATUS_TONE` uses on every lead card. Closed
leads get **no** red/green treatment — this design system spends colour on state
you must act on, and a closed lead is not that. The bar is `aria-hidden`; the
percentage beside it is the accessible value, so a screen reader hears the
number once.

**No role gate, deliberately.** Every MEMBER already reads every lead in the
tenant, so an aggregate of those same rows may be neither narrower nor wider
than the lists they can already open. RLS plus the explicit `organization_id`
filter is the whole boundary; the page adds none of its own.

**Two deliberate v1 non-goals, registered rather than forgotten.** There is no
date-range or period filter — the view is all-time only, because a "this month"
toggle carries its own edge cases (tenant timezone, partial months, which
timestamp a closed lead is bucketed by) and each is a real decision, not a
parameter. And the report itself cannot be exported; the CSV work in Prompts
9-10 shares no machinery with it, since that exports rows and this would export
an aggregate. Both are in `docs/KNOWN_GAPS.md`.

**Verified.** `npm run build`, `npm run lint` and `tsc --noEmit` all clean.
Numbers cross-checked against a SELECT-only MCP query over the demo org
(`cdf8217a-…`): open 16 leads / $146,750 (NEW 5/$41,050/28%, DISCOVERY
4/$23,800/16%, QUOTED 4/$57,900/39%, ACTIVE 3/$24,000/16%), WON 2/$30,300,
LOST 1, ABANDONED 1, win rate 67% — every figure identical on screen.
Screenshotted in both themes through the dev-login route, keyboard tab confirmed
to reach the new nav link with a visible focus ring, and the "More" sheet
confirmed to list Reports while the tab bar kept its four slots. Read-only work,
so there is no test residue to remove.

## 2026-08-26 — handoff attach-list: a budget, not an inventory

The handoff block's `### Attach to this Project` line had gone stale in the one
way its own check set could not see. It told the planning Project to attach
`DESIGN.md` and `docs/SCHEMA_REFERENCE.md` permanently and to never attach
`docs/ADDENDA_LOG.md` — a list written before the 2026-08-18 split, when the log
was a single ~419 KB / ~103k-token file and genuinely too expensive to park in
Project knowledge. After the split the log is a ~12 KB index and is the
*cheapest* file in the set, while DESIGN.md and SCHEMA_REFERENCE.md together are
~26k tokens that most planning chats never open. Project knowledge loads into
every conversation in the Project, so the old five-file set cost ~146 KB / ~36k
tokens on every chat regardless of what that chat was about.

The failure mode that made it worth fixing rather than tolerating: the block is
printed fresh by every handoff, so **each run would silently revert a correction
the owner had made by hand in the Project.** A stale template that reprints
itself is not the same class of problem as a stale doc that merely sits there.

Replaced with three tiers — permanent (`CLAUDE.md`, `docs/KNOWN_GAPS.md`,
`docs/ADDENDA_LOG.md`), per chat only when that chat needs it
(`docs/SCHEMA_REFERENCE.md` for schema/RLS/migration planning, `DESIGN.md`
for a visual-design argument), never (`docs/addenda/*.md`, ~480 KB of dated
narrative — name a section and it gets pasted). The closing rule now states the
budget reasoning instead of calling the list an inventory, so a later reader can
place a new doc in the right tier rather than appending to a flat list.

Four smaller repairs in the same pass, all of the same shape — the skill
describing itself wrongly:

- The block's exclusion gate cited "Check 11" for the permanently-rejected
  filter. That is **check 13**; check 11 is reference integrity. A reader
  following the pointer would have landed on the wrong check and skipped the
  filter entirely — and that filter is the only guard on
  `### Needs the user, not more code`, which is drafted by hand.
- "68 sections … 70 now" for the addenda was stale; there are **77**
  (26 + 40 + 3 + 8), matching `check-section-pointers.mjs`'s own output.
- Checks 9–11 have been wired as `npm run check:docs` since they were written,
  and the skill never said so — every run invoked three `node` commands by hand.
- Documented the one known `check-doc-figures.mjs` flake: vitest cold-starts on a
  session's first run and the script reports `CANNOT RUN`, which the skill's own
  "exit 2 is not a pass" rule would otherwise turn into a hunt for a bug that is
  not there. Run `npm run check:docs` twice before believing it.
- The skill's frontmatter omitted `docs/SCHEMA_REFERENCE.md`, which check 10
  audits.

Note the shape of all six: not one is a stale *document*. Every one is the
**audit instrument** describing itself incorrectly, which is why checks 1–12 —
which point outward at the repo — could none of them catch any of it. There is
no script for this and there should not be one; the instrument is repaired when
a run notices it is wrong.

`npm run check:docs` green after: 12 colour tokens × 2 themes, 8 type roles,
5 radius steps; 143 tests / 23 suites; 14 `SECURITY DEFINER` functions; 253
section pointers, 77 index rows, 77 addenda sections all indexed.


## 2026-08-26 — `prospects`: cold-outreach staging, its RLS and its CSV import

Prompt 1 of 2. The sibling `tekguyz-leadgen` repo scrapes Google Business
Profiles; 122 of them were sitting in two CSVs in `C:/Projects/tekguyz-leadgen/output`
with nowhere to go. GBP does not expose an email address and never will, and
`leads.email` is `NOT NULL` and half of `unique_tenant_client_email_ci` — so this
data structurally cannot enter `leads` without either weakening that index or
synthesizing fake emails. Both reopen the identity-collision bug class that
`lead_submissions` was built to close.

Migration: `supabase/migrations/20260826120000_prospects.sql`. One table, one
trigger, three RLS policies, three indexes, one `SECURITY DEFINER` RPC. **Nothing
on `leads` was touched** — DDL, the three RLS policies, `unique_tenant_client_email_ci`,
both lead triggers and `import_leads_chunk` are all byte-for-byte unchanged, and
re-verified live after the migration (25 `leads` columns, 3 `leads` policies,
`import_leads_chunk` still present).

### The dedup key is `place_id`, not a phone or a name

Google's Place ID is leadgen's own global, append-only key. `unique_tenant_place_id
(organization_id, place_id)` is what makes re-importing a scrape a no-op: the RPC's
`ON CONFLICT … DO NOTHING` means a prospect already marked `CALLED` or
`NOT_INTERESTED` is never reset by a stale CSV. Scoped by tenant, not global —
two organizations may each legitimately hold the same business.

### Phone normalization is one expression written twice, on purpose

`prospects.phone_digits` is a **stored generated column**, so it can never
disagree with `phone` and no caller has to remember to set it. The identical
expression is written a second time on the `leads` side of the duplicate lookup
inside the RPC, because `leads` could not get a column in this unit. That
duplication is deliberate and documented at both sites; a shared `IMMUTABLE`
helper function was considered and rejected as the riskier option, since a
generated column that depends on a function is painful to change later.

`NULL` means **not comparable**, not "no digits". Anything under ten digits is
dropped, because a 7-digit local number would false-match every number ending in
those same seven. Ten digits is the NANP subscriber number, and taking the *last*
ten also strips a leading `1`/`+1`. `src/lib/prospects/phone.ts` is the TypeScript
twin, unit-tested; the RLS suite asserts the SQL side behaviourally, so drift
fails a test rather than silently missing duplicates.

`possible_duplicate_lead_id` is **reference only** — a `LEFT JOIN LATERAL`
written once at import time, never a constraint and never a dedup key. A shared
number can be a shared answering service or a stale lead, so the judgement stays
with a human. The lookup is scoped to `p_organization_id`, which matters more
than it looks: `SECURITY DEFINER` bypasses RLS, so an unscoped lookup would be a
cross-tenant phone-number oracle — import a number, see whether it comes back
flagged. There is a test for exactly that.

### Dry run before the file was applied

Per CLAUDE.md § Session & Verification Discipline, the whole thing ran first
against a temp-table replica (`pg_temp` function, `ON COMMIT DROP` temp tables,
no `public`-schema object touched). It proved the four things a review cannot:
the generated expression compiles with `pg_catalog."right"` quoted (RIGHT is a
reserved word), the `ON CONFLICT` target does not collide with the OUT column
names, 5 → 0 on re-import, 2-of-3 on a partial overlap, and every phone format
collapsing to the same ten digits. Only then was the real file applied.

### Real numbers, from a real import through the real UI

Both leadgen CSVs, dropped into `/prospects/import` while signed in through
`GET /api/dev-login`:

| File | Rows in file | Inserted | Already present | Repeated in file | Phone matches a lead |
|---|---|---|---|---|---|
| `all-niches_fort-lauderdale-fl_2026-08-25T08-42-17Z.csv` | 91 | 91 | 0 | 0 | 0 |
| `plumber_fort-worth-tx_2026-08-25T07-55-34Z.csv` | 31 | 31 | 0 | 0 | 0 |
| the same two files again | 91 / 31 | **0 / 0** | **91 / 31** | 0 | 0 |

122 rows, 122 distinct `place_id`s, 117 with a usable `phone_digits` and 5
without — matching the files exactly (5 of the 91 scraped rows have no phone).

**The flag count of 0 is real, not a broken lookup.** An independent
re-computation over the live tables — every prospect joined to every same-tenant
lead on the normalized digits — also returns 0, against 21 leads that do have a
usable phone. The lookup itself is proven to fire by six RLS-suite cases and by
the dry run, in both directions of formatting.

### Verification

`npm run test:rls`: **64/64, 4 files.** 43 of those are the pre-existing
`leads`/`organization_members` suites, unchanged and still passing — this table's
arrival touched nothing else. 21 are new, in
`src/lib/prospects/prospects.rls.test.ts`: member parity, the check constraint,
cross-tenant read/insert/update, the paired `WITH CHECK` (a member cannot move
their own row into another tenant), no `DELETE` reachable at all, the RPC
rejecting an outsider and an anonymous caller, a **forged `organization_id` on
the row payload being structurally unreachable**, idempotency, `DO NOTHING` not
resetting operator state, the same `place_id` in two tenants, the generated
column across seven phone formats, both directions of the duplicate hint, and
the cross-tenant non-match.

`npm test`: 146/24 (was 143/23 — `src/lib/prospects/phone.test.ts` adds 3).
`npx tsc --noEmit` and `eslint` clean. `npm run build` green, `/prospects/import`
at 2.53 kB.

`db advisors --type security`: one new line, `import_prospects_chunk` under
`authenticated_security_definer_function_executable` — the **same WARN class every
existing RPC in this project already carries**, and the disposition for that whole
class is already "rejected, never re-list" in `docs/KNOWN_GAPS.md`. `anon` does
not appear for it, which is the revoke working. No new performance findings.

### Test residue

The RLS suite deletes its own two throwaway organizations and three auth users
in `afterAll` — cascading to every prospect and lead it created. The two CSVs
were staged under `public/__tmp_prospect_verify/` so the browser could fetch them
(a file input cannot be populated from outside the page); that directory was
removed. The 122 real prospects first landed in **TEKGUYZ Demo** rather than TEKGUYZ,
because `/api/dev-login` signs in as the seeded demo owner and there is no way
for an agent to authenticate as `admin@tekguyz.com`. **Closed the same day**:
the owner signed in as themselves, re-ran both files, and ran the scoped
`DELETE` in Studio — the agent sandbox refused the write, which is the outcome
CLAUDE.md § Test-Data Cleanup asks for anyway.

### Close-out, verified against the live database rather than taken on report

`TEKGUYZ`: **122 prospects, 122 distinct `place_id`s, 117 with a usable
`phone_digits`, 5 without, 0 flagged**, all at `status = 'NEW'`, split **91 Fort
Lauderdale, FL / 31 Fort Worth, TX**. `TEKGUYZ Demo`: **0 prospects**, its **20
leads untouched** (21 leads project-wide, unchanged all day). Two organizations
only; no throwaway org and no `rls-place-*` fixture survived.
`npm run check:residue` clean.

One thing worth not mistaking for a bug later: the 91-row export carries **12
distinct `run_id`s summing to 91**. `all-niches` is a merge of several scrape
runs against one city, so several run ids in one file is the normal shape, not
duplication — the 122 distinct `place_id`s are what proves there is none.

## 2026-08-26 — Prospect promotion: the write path into `leads`

Prompt 2 of 2 of the Prospects initiative. Adds the two columns Prompt 1 did not
need, a `/prospects` call list, inline editing, and a Promote action that turns a
prospect into a real lead. Migration
`supabase/migrations/20260826130000_prospects_promotion.sql`, applied by hand
through the Supabase SQL editor and live-verified the same day.

### The non-negotiable: one lead-write path, not two

`createLead` used to hold the `leads` INSERT and its paired
`recordLeadSubmission` call inline. Promotion needed the same pair **plus the new
lead's id**, which a Server Action returning `LeadFormState` cannot give it.

The fix was a minimal, additive extraction rather than a second write path.
`src/lib/leads/create.ts` is new and holds `insertLeadWithSubmission(supabase,
organizationId, input)`, lifted verbatim out of `createLead`. `createLead` now
calls it and is otherwise unchanged — same columns written, same submission
recorded, same `LeadFormState` returned, same `CreateLeadModal` field set. Its
diff is the import line, the removal of the inline insert/submission block, and
one `if (!result.ok) return { error: result.error.message }` branch.

`insertLeadWithSubmission` accepts two fields `createLead` does not pass:
`physicalAddress` (a prospect knows its street address; the manual form has no
such input) and `message` (a cold call genuinely produced words, so the
`lead_submissions` row carries them — `createLead` still passes `null`, because
inventing a message for a typed-in lead would be a fake record). Both are
optional and default to `null`, so nothing existing changed behaviour. The
webhook, CSV import and demo seed do not call this function at all; they keep
their own service-role paths, which were deliberately not consolidated into it.

### `promoted_lead_id` is the only source of truth, and the guard is one statement

`prospects.status` already had `'CONVERTED'` from Prompt 1. It is a label an
operator can set by hand from a dropdown and it cannot carry the identity of the
lead, so it can never answer "has this been promoted". `promoted_lead_id` can,
and only the promotion path writes it.

The claim is a single statement, which is a single transaction:
...
One statement means the two columns cannot disagree. The `is null` predicate
means a second promotion affects **zero rows** rather than overwriting the first
one's lead id — proven live, including that the original `promoted_lead_id`
survives untouched. Every read that asks whether a prospect can still be promoted
tests `promoted_lead_id`, never the status string; a prospect whose status was
hand-set to `CALLED` is still promotable, and that has its own test.

`CONVERTED` is therefore **not** offered in the operator dropdown.
`OPERATOR_PROSPECT_STATUSES` lists four values and `setProspectStatus`
re-validates against them server-side, because a Server Action is a public HTTP
endpoint. That action also carries `.is("promoted_lead_id", null)`, so a promoted
prospect's status cannot be walked back off `CONVERTED` while it still points at
a real lead.

### The zero-rows case is handled explicitly, and it can orphan a lead

If the guarded update matches nothing, a concurrent promotion won the race
between the pre-check and the claim. The lead created moments earlier is real and
is now unreferenced. `promoteProspect` names that rather than hiding it: it logs
the orphaned lead id server-side and returns a message telling the operator a
duplicate lead was created, with a link to the original. Silently succeeding
would leave a duplicate lead nobody knows about; a generic throw would leave the
operator unable to tell which lead is the good one.

Full cross-table atomicity would need a `SECURITY DEFINER` RPC wrapping the lead
insert, the submission insert and the claim in one transaction. That was out of
scope — the migration was fenced to two additive columns on `prospects` — and the
race window is one round trip wide. Registered in `docs/KNOWN_GAPS.md`.

### `possible_duplicate_lead_id` links, never blocks

Surfaced on the row as "Phone matches an existing lead", pointing at that lead.
Promote stays enabled beside it. A shared switchboard number is an ordinary
reason for two different businesses to collide on a phone match, so a block here
would stop real work on a guess.

### Email collision

`unique_tenant_client_email_ci` is the one collision this path can hit.
`isEmailCollision` recognises `23505` and the constraint name, the action looks
the existing lead up by a wildcard-free `ilike` (case-insensitive equality, since
rows predating `20260726120000_case_insensitive_email.sql` may not be
lowercased), and the modal renders a specific message plus a link to that lead.
Triggered for real during verification:

> A lead already uses promote-verify-01@example.invalid. Open it and add the call
> notes there instead, or promote this prospect with a different email.

Confirmed live that a failed insert creates **no** lead — two deliberate
collisions left exactly one lead in the database, the one that succeeded.

### A React 19 bug found in the browser, not in tests

The Promote form was first written with `defaultValue` on every field. React 19
**resets a `<form action={...}>` after the action returns, failure included** — so
the collision error arrived with the typed email already wiped and every
corrected field reverted to its prefill, at exactly the moment the message was
telling the operator to change something. `tsc`, `eslint`, `next build` and every
unit test were green throughout.

Every field is now controlled from one `values` state object seeded by
`prospectToPromoteDefaults`. Two regression tests pin it: typed values survive a
re-render, and no field renders a `defaultValue` attribute. Re-verified in the
browser — the typed email now survives the collision.

### Field parity, mechanically

`PROMOTE_FIELD_NAMES` in `src/lib/prospects/promote-payload.ts` is the single
declaration of the form's field set. Parity is diffed in **three** directions,
because the failure mode is silent (CLAUDE.md § Form/Action Field Parity):

1. every `formData.get()` in `buildPromotePayload` is in the list,
2. every name in the list is read by `buildPromotePayload`,
3. `PromoteProspectModal` renders exactly the list, each name exactly once —
   asserted against the real rendered DOM, not against the source.

Eleven fields each way. The prefill helper lives in the same file as the parser
on purpose: a form split across siblings hides its own field set, which is what
hid the NULL-on-save bug twice.

### What shipped elsewhere

- `/prospects` (new route) with `loading.tsx`; sort, text filter, status filter,
  Active/Archived tabs. Sorting and filtering are client-side over the fetched
  array — a call list is worked through between calls, and a server round trip
  per header click would lose scroll position. Nulls sort **last in both
  directions**: a business with no rating is not the worst-rated business.
- `SECONDARY_NAV`'s "Prospects" entry retargeted from `/prospects/import` to
  `/prospects`; the importer is reached by a button on the list page. The mobile
  tab bar's four slots were not reopened.
- Inline `status` / `notes` / `archived` editing via three Server Actions in
  `src/lib/actions/prospect-actions.ts` (a NEW file; `prospect-import-actions.ts`
  and `import-actions.ts` untouched). Typed arguments, not `FormData` — there is
  no form here, so there is no field set to drift.
- Seven synthetic demo prospects in `scripts/seed/lib/demo-prospects.ts`, wired
  into both `seed:demo` and `seed:demo:reset`. Obviously fake by construction:
  `demo-place-####` place ids (a real Google Place ID starts `ChIJ`),
  transparently invented names, `555-01xx` phones, `.invalid` websites. One is
  archived, one has no phone, one has no address, none is `CONVERTED`. The two
  real leadgen CSVs were **not** re-imported; the 122 real prospects in TEKGUYZ
  were confirmed untouched after every step.
- `src/test/setup.ts` gained an `HTMLDialogElement.prototype.showModal`/`close`
  stub. jsdom implements `<dialog>` as an element but not its modal behaviour, so
  any test rendering a `Modal`-based component threw before its first assertion.

### Two things worth not repeating

**A `"use server"` file may only export async functions.** The status constants
started life in `prospect-actions.ts`. `tsc` and `eslint` passed; the production
build failed at page-data collection with "A 'use server' file can only export
async functions, found object". They moved to `src/lib/prospects/statuses.ts`.

**The migration was NOT dry-run against a temp-table replica.** That check has
always run through the Supabase MCP's `execute_sql`, and the MCP is deliberately
not authorized on this project — its OAuth flow grants `database:write`,
`projects:write`, `secrets:read` and `storage:write`, and CLAUDE.md already
forbids using any of it, so the scopes would be granted and never legitimately
used. **The Supabase CLI is the tool for schema work here, not the MCP.** The
replica check exists to catch traps in functions and policies — the ambiguous
`OUT` column class — and this migration has no function, no policy, no constraint
change and no `ON CONFLICT`: two `add column ... null` statements and one index.
Skipping it was accepted deliberately by the user, and is recorded here rather
than passed over in silence.

**A separate finding, not this feature's.** `supabase db push` refused, and
`supabase migration list` showed why: the remote history table holds exactly one
row (`20260707215206`, which has no local file) while all 26 local migrations
show as un-applied. They *are* applied — by hand, outside the CLI. The database
is correct and the bookkeeping table is empty, so a push would try to replay
everything from scratch and `create table public.prospects` would fail. Logged by
the user as its own chore rather than repaired in the middle of a feature.

### Verification

- Migration applied by hand, then confirmed live: selecting `notes` and
  `promoted_lead_id` succeeds (PostgREST errors `42703` on a column that does not
  exist, so the select IS the test), 122 real prospects intact, `leads` unchanged.
- `npm test` — **179 passed, 27 files** (146/24 before; +15 promote-payload,
  +11 sort, +7 modal).
- `npm run test:rls` — **76 passed, 5 files** (64/4 before, all 64 unchanged;
  +12 in `src/lib/prospects/prospect-promotion.rls.test.ts`).
- `npm run build` green, `npm run lint` clean.
- Real browser, signed in via `GET /api/dev-login`: promoted a demo prospect and
  confirmed in the database that the lead carries every first-known identity
  column, that exactly one `lead_submissions` row exists for it, and that the
  prospect is `CONVERTED` with a matching `promoted_lead_id`. The promoted row
  then renders a "Converted" badge and a "View lead" link, with no Promote button.
- Keyboard: every control is a native `a`/`button`/`input`/`select`, no
  `tabindex="-1"` anywhere in `main`, and a real Tab sweep walks search → status
  filter → the four sort headers → phone link → notes → View lead.
- Both themes rendered and screenshotted.
- Test residue removed: `npm run seed:demo:reset` after verification, and both
  disposable scripts deleted.

Two RLS runs failed to start with "JWT issued at future" and passed on retry —
clock skew between this machine and the hosted project (the local clock measured
29 s behind real time), not a code failure.

## 2026-08-26 — Controlled fields: closing the React 19 form-reset data loss

Closes the Known Gaps bullet opened the same day, when `PromoteProspectModal`
was fixed in isolation and the same shape was found in six other surfaces.

React 19 resets a `<form action={…}>` after the action returns — **failure
included** — by calling `form.reset()` on the element. An uncontrolled form
therefore wipes what the user typed at exactly the moment the error message is
telling them to fix one field. Green build, passing tests, invisible except in a
browser.

### What was converted

Seven files, all to the plain `useState` pattern `PromoteProspectModal` already
shipped — no form library, every `name=` byte-for-byte unchanged, so the
`formData` shape each action receives is identical:

`CreateLeadModal` (8 fields) · `InviteMemberForm` (2) · `OrgDetailsPanel` (3) ·
`AccountPanel` (display name + 2 checkboxes) · `ApiKeysPanel` (2) · `TaskRow`
(title + description; its due-date field was **already** controlled and was not
touched) · and `EditLeadModal` — which is where the interesting part is.

### `EditLeadModal` is one form across five files, and the reset does not respect that

The prompt scoped this surface as "`EditLeadModal` (via its `IdentityFields`
sibling)". That is not enough to fix it. `EditLeadModal` owns a single
`<form action={…}>` whose fields are supplied by **five** sibling components —
`IdentityFields`, `AddressSocialFields`, `PipelineFields`, `AssignmentField`,
`OutcomeFields`. `form.reset()` hits every field in the form regardless of which
file rendered it, so converting one sibling leaves nine fields still losing their
edits in the same modal. All five were converted, and the regression test asserts
the **whole form at once** rather than any one file — which is the only level at
which the property is meaningful. This is the same "no single file shows its own
field set" hazard CLAUDE.md § Form/Action Field Parity already warns about,
arriving from a new direction.

### Controlling a field is only half the fix — two parts nothing warned about

Found in the browser, not in a test, and this is the substance of the session:

**A controlled `<select>` is not restored.** React restores a controlled
`<input>` after the reset by itself. It does not do so for `<select>`:
`form.reset()` moves the DOM selection behind React's back, React's state is
unchanged, so no re-render is scheduled and nothing writes the value back.
Measured live against a real `unique_tenant_client_email_ci` failure — the
Outcome select's React props read `"WON"` while the DOM element read `""`. State
correct, display wrong, and the *next* submission would have posted the wrong
value, because `FormData` reads the DOM.

**A Radix Checkbox is actively dragged back.** `@radix-ui/react-checkbox` 1.3.11
registers its own `reset` listener on the enclosing form and calls
`setChecked(<value at mount>)`; for a controlled checkbox that fires
`onCheckedChange` with the OLD value. Read out of the installed package, not
assumed.

Both are handled by one shared hook, `src/lib/forms/use-form-reset-restore.ts`,
rather than a copy per component — the same "never hand-copy a mechanism" rule
the primitives live under. Three details in it are load-bearing:

- the work runs in a **microtask**, so it lands after every reset listener
  whatever order they registered in — the one thing that must not depend on
  Radix's internals staying as they are;
- it finds the form by walking up from an anchor ref rather than being handed
  one, because a field group often does not own the `<form>` (four of
  `EditLeadModal`'s five siblings do not);
- a checkbox's user intent is recorded from **`onClick`**, never
  `onCheckedChange`. Radix's restore drives `onCheckedChange` too, so recording
  there overwrites the choice with the value being restored and makes the
  restore a no-op. That mistake was made first and caught by the failing test.

### The test that nearly shipped green while wrong

The first version of every regression test did `form.reset()` **and then**
`rerender(...)`. It passed. It was wrong: RTL's `rerender` forces a render that
re-applies the controlled value, which React 19 does not necessarily do in the
real app, since the component's props have not changed. That masked the entire
`<select>` failure above — nine tests green, the bug still live in the browser.
The tests now call `form.reset()` and assert through `waitFor` with **no**
`rerender`, which is the honest model, and each one was re-proven to fail against
the original code after the change.

`PromoteProspectModal.test.tsx` carried a third instance of the same mistake, and
a worse one: an assertion that could **never** fail. It filtered the form's fields
for a `defaultValue` **attribute** in the DOM, and React never emits one — it
reads `false` for a controlled and an uncontrolled input alike, so it passed
against both. That is where CLAUDE.md's old "assert it: a React-controlled field
renders no `defaultValue` attribute in the DOM" wording came from, which is why
the rule text was corrected in the same pass rather than left to teach the next
reader to write a test that cannot fail. The assertion was replaced with a real
`form.reset()` survival test covering all four typed fields plus the hidden
`prospect_id` the whole write is keyed on, and that replacement was itself proven
to fail — `client_name` was temporarily reverted to `defaultValue` and the test
went red before the component was restored.

### Verification

Both gates and the browser, not one or the other:

- **211 tests, 34 suites** (`npx vitest run`), up from 179/27. Nine new
  reset-survival tests plus supporting parity/prefill coverage across seven new
  suites; every one of the nine was observed failing against the pre-fix code
  via `git stash`, twice — once before the `rerender` correction and again after.
- `npm run build`, `npm run lint`, `npx tsc --noEmit` all exit 0.
- **Live, signed in via `GET /api/dev-login`.** `CreateLeadModal`: a genuine
  `unique_tenant_client_email_ci` collision left the modal open with all eight
  typed values intact. `EditLeadModal`: the same real error, with the Outcome
  select — set by a real keypress — reading `WON` after the failure, the case
  that was broken before the hook existed.
- No schema, migration, RLS policy or Server Action was touched, as scoped.
- One invite row created while testing `InviteMemberForm` was removed with a
  disposable service-role script, which was then deleted; `npm run check:residue`
  reports clean, `TEKGUYZ Demo` still holds 20 leads, and the lead used for the
  edit test is byte-identical to its seeded row.

## 2026-08-27 — Three UI defects: double scrollbar, the missing Maps link, the raw timezone id

Three confirmed defects, fixed as one unit. No schema change was needed for any
of them.

### 1. The whole document scrolled underneath the app's own scroll container

**Symptom:** on any long view — `/prospects` with a real 122-row tenant,
`/settings` at any size — scrolling ran roughly twice as far as the content.
Reaching the bottom of the content did not stop the page.

**Root cause.** `AppShell`'s `<main>` was `overflow-y-auto` but *not
positioned*. An absolutely-positioned descendant is clipped by an ancestor's
overflow only when that ancestor is in its **containing-block chain**, and with
`main` static the chain skipped it and landed on the shell's `relative` div —
which is outside the scroller. Those descendants therefore contributed their
full offset to the **document's** scroll height, so `documentElement.scrollHeight`
grew to the height of the content inside `main` and the window itself scrolled.
Measured before the fix on `/settings`: `scrollingElement.scrollHeight` 1664
against `clientHeight` 703, with `window.scrollY` reaching 959 while `main`
independently scrolled 1127.

The escapees are ordinary, which is why this hit every long page rather than
one component:

- **Every `.sr-only` span.** Tailwind's `sr-only` is `position: absolute`. The
  archive button in `ProspectRow` carries one per row.
- **Radix `Checkbox`'s hidden bubble `<input>`**, which it styles
  `position: absolute` inline. `AccountPanel`'s two notification checkboxes are
  the `/settings` case; confirmed by reading the live element's computed
  `offsetParent`, which was the shell div and is now `MAIN`.

**Fix:** one class — `relative` on `<main>` in
`src/components/shell/AppShell.tsx`. That makes `main` the containing block for
its own absolute descendants, so the scroller clips them again. Verified live:
`/prospects` padded to 31 rows and `/settings` at its real length both report
`scrollHeight === clientHeight` on the document, `window.scrollY` pinned at 0
after a 5000px scroll attempt, and `main` scrolling to exactly its content end
(24px of `p-6` padding left below the last element, as intended).

**The rule this leaves behind** is in `CLAUDE.md`: the shell's scroll container
must stay positioned. It is not decoration, and removing it re-opens this with
a green build and passing tests.

### 2. The scraped GBP listing was reachable from nowhere in the UI

`prospects.google_maps_url` **already existed** — it landed with the original
table in `20260826120000_prospects.sql`, is in the CSV header contract in
`src/lib/validation/csv-prospect-schema.ts`, is written by
`import_prospects_chunk`, and is already selected by `PROSPECT_COLUMNS`. **No
migration was needed.** Nothing rendered it.

`ProspectRow` now renders an icon-only `IconBrandGoogleMaps` link inline beside
the business name, through `Button asChild` (never a hand-copied class string),
`target="_blank" rel="noopener noreferrer"`, with the visible label in an
`sr-only` span and a `title` for pointer users. Inline rather than its own
column: the table is already seven columns wide and a column would spend
horizontal room on every row to carry one link. Measured after the change at a
1440px viewport: rows carrying the link are 55px tall against 54px for the row
without one, and the business name still occupies a single line box.

The demo fixtures in `scripts/seed/lib/demo-prospects.ts` had no
`google_maps_url` at all, so the demo org could not exercise the new link.
Six of the seven now carry a maps **search** URL for their invented name — not
a `/place/` URL with a fabricated place id — and `demo-place-0003` deliberately
keeps `null` so the no-listing row proves it renders no dead link. The seven
existing rows were backfilled with a disposable service-role script, deleted in
the same session per the Test-Data Cleanup rule.

### 3. The timezone select showed a machine identifier

`OrgDetailsPanel` rendered the raw IANA id as the option label, so an operator
picking a timezone read `America/New_York`. `timezoneLabel()` in
`src/lib/organizations/org-options.ts` is presentation-only: the option `value`
is still the exact IANA string the server action validates against, and the
stored column is untouched. It takes the final path segment and replaces
underscores. All 55 curated zones have a distinct final segment, so the region
prefix is dropped rather than parenthesised — `src/lib/organizations/org-options.test.ts`
pins that uniqueness, so a future zone whose city collides fails a test instead
of shipping an ambiguous label.

**Verification.** `npx tsc --noEmit` clean, `npm run lint` clean,
`npm run build` clean, `npx vitest run` **216 tests / 35 suites** (was 211/34;
the five new ones are the timezone label suite). Browser verification for all
three via `GET /api/dev-login` as the demo owner.

## 2026-08-28 — Command palette: tasks alongside contacts

The ⌘K palette fetched one source and its placeholder read "Search
contacts…", but its trigger sits in `AppShell` on every route — so it
presented as app-wide search while covering roughly a quarter of what a user
would reach for. This adds a second source and groups the results.

**No schema, no migration, no policy change.** Both `leads` and `tasks` are
already covered by plain `organization_id IN (SELECT private.current_org_ids())`
RLS, so the new query is tenant-scoped by the same boundary every other read
uses. Cross-tenant isolation was re-confirmed live with an identically-named
task in the other org, which did not appear.

### The new query and its three filter decisions

`searchTasksForOrg` in `src/lib/tasks/queries.ts`:

    .select("id, title, due_at, completed, lead_id,
             leads!inner(client_name, archived)")
    .eq("organization_id", orgId)
    .eq("dismissed", false)
    .eq("leads.archived", false)

**`dismissed = false`.** Dismissal is removal from every active surface — the
role `archived` plays for a lead — so a dismissed task is out of search for the
same reason an archived lead is.

**No `completed` filter, deliberately.** Completed is ordinary lifecycle state,
not removal, and a finished task is legitimate history to search for. This is
the precedent the Contacts directory already sets by showing WON/LOST/ABANDONED
(CLAUDE.md's Contacts Directory Scope rule). `getTasksDueForOrg` **does** filter `completed`,
because an agenda is a worklist rather than a directory — the two queries must
not be assumed to share a filter shape, and that is the trap this note exists
to pin.

**`leads!inner` + `leads.archived = false`, and this one is not redundant.**
`closeTasksForArchivedLead` writes `completed = true`, never `dismissed`, so an
archived lead's tasks keep `dismissed = false`. With no `completed` filter above
to incidentally hide them, this join is the only thing keeping an archived
lead's tasks out of global search.

### Selecting a task

Selecting a task opens its parent lead's profile sheet with that task scrolled
to and briefly marked, threaded as a prop — `CommandBar` already opens the sheet
from local state, and the `?leadId=` URL path is a separate component serving
email links, left untouched.

`TasksSection` switches its Open/Completed tab to match **before** scrolling: a
completed task is not in the DOM at all under the default Open tab, so the
scroll would otherwise find nothing. That tab switch is the one behaviour proven
to fail against the pre-fix code rather than only to pass against the fix.

The arrival marker reuses `OptionRow`'s `--accent` bar over `--canvas-soft`
rather than inventing a second "this is the one" idiom, and clears on a timer so
it never reads as persistent selected state. Geometry and tokens measured in
both themes.

### Verification

231/231 on `npm test` (36 suites), `npm run check:docs` and
`npm run check:residue` both green. Verified live against the seeded demo org,
all four cases in one search: an open task and a completed task both returned;
a dismissed task and a task on an archived lead both absent. All verification
rows removed per CLAUDE.md § Test-Data Cleanup.

## 2026-08-28 — The loading-skeleton flash: a fixed auth tax on every navigation

Reported by the product owner: a loading skeleton flashes on **every**
client-side navigation, including routes with almost no data, and its length
does not track how much the route actually loads. Diagnosed cold — no cause
assumed.

### What it was not

Three plausible causes were checked and ruled out against the real code before
anything was changed. There is no `template.tsx` anywhere under `src/app/`
(one would remount the subtree on every navigation). No route sets
`dynamic`/`revalidate`/`fetchCache`. And the shared `(app)` layout is **not**
re-executing per navigation — a render probe in `(app)/layout.tsx` printed on
document loads and stayed silent across client navigations, which is correct
Next behaviour and meant the layout's `getTeamMembers` was never on the hot
path.

Two false trails are worth recording because both cost time and both look like
findings.

**The hidden Browser pane produced a fake root cause.** With the pane not
compositing, every nav click performed a *hard document navigation*: no
`?_rsc=` request, `defaultPrevented: false` at the document's bubble phase, and
no React fiber on the anchor — which reads exactly like "the app never
hydrated, so `<Link>` is inert". It is the trap already documented in CLAUDE.md
§ Session & Verification Discipline: the streamed content was still parked in
`<div hidden id="S:0">` holders, so the anchors being queried were not in the
React tree at all. Driving a real headless Chrome over CDP instead (Node 24 has
a native `WebSocket`, so this needs no dependency) showed proper soft
navigation on every route.

**Dev-mode measurements pointed at the wrong thing entirely.** Next disables
`<Link>` prefetching in development. In `next dev` the skeleton appeared ~500ms
after the click — *after* the server responded — which reads as a slow fetch.
In a production build it appears **~55ms** after the click, before any data has
been asked for, because the prefetch has already delivered the `loading.tsx`
fallback and only the fallback. Same code, opposite conclusion. This is now a
permanent rule in CLAUDE.md § Session & Verification Discipline.

### The actual root cause

Measured against `next build` + `next start`, signing in through the real
`/login` form, 12 navigations across four routes:

| | skeleton appears | skeleton visible (median) |
|---|---|---|
| Baseline | 55ms after click | **759ms** |
| After the fix | 64ms after click | **345ms** |

The number that identifies the cause is not the size but the **flatness**:
`/settings` (14 skeleton blocks), `/reports` (21), `/contacts` (40) and
`/pipeline` (72) all sat within ~20ms of each other at ~750ms. A cost that does
not vary with the data is not a data cost.

It was authentication, paid twice per navigation and once more per document
load:

1. `updateSession` in the middleware called `supabase.auth.getUser()` — a
   network round-trip to the Supabase Auth server — on every request the
   matcher covers, RSC navigations included. Worth ~125ms of TTFB (149ms → 25ms
   once removed).
2. `getCurrentOrg()` called `getUser()` again on the same request, then ran
   **two serial queries** (memberships, then organizations — the second could
   not start until the first named the org id).
3. `getCurrentOrg()` was not memoised, and both `(app)/layout.tsx` and every
   page call it, so a document load ran the whole thing twice concurrently
   (measured at 663ms and 683ms for one request).

`loading.tsx` was never the problem, and a control proved it: a zero-data page
added under `(app)` with its own `loading.tsx` made **no RSC request and showed
no skeleton at all** — a static segment is prefetched whole, so there is nothing
to wait for. The skeleton is only ever as long as the server takes.

### The fix

Three changes, no schema, no migration, no RLS, no policy.

- **`src/lib/supabase/middleware.ts`** — `getUser()` → `getClaims()`.
- **`src/lib/organizations/current.ts`** — `getUser()` → `getClaims()`; wrapped
  in React's `cache()`; the organization is now a PostgREST embed on the
  membership select instead of a second serial query.
- **`src/lib/account/actions.ts`** — `updateDisplayName` calls
  `supabase.auth.refreshSession()` after `updateUser` and before
  `revalidatePath`.

That last one is the deliberate cost of the first two, not an optimisation.
`getClaims()` reads `user_metadata` out of the JWT, `updateUser` does **not**
reissue the token, and the token lives an hour — so without the refresh a
renamed user would keep seeing their old name and the rename would look broken.
Verified against a live token rather than assumed: the access token is `ES256`
and carries `sub`, `email` and `user_metadata`, so every field `getCurrentOrg`
reads is genuinely present. `@supabase/auth-js` caches the JWKS in a
module-level `GLOBAL_JWKS` keyed by storage key, so the local verification does
not re-fetch per request even though `createClient()` builds a fresh client
each time.

`getClaims()` is not `getSession()`. It verifies the signature; `getSession()`
trusts the cookie. The security guarantee is unchanged from `getUser()`.

### What the fix does not do

**The flash is shorter, not gone — 12/12 navigations still show a skeleton.**
Next paints the prefetched `loading.tsx` the instant the link is clicked, so
any dynamic route will always show it for however long the server takes. The
remaining ~345ms is the page's own queries, which is honest load time.

Removing a route's `loading.tsx` does **not** remove the skeleton, and this was
measured rather than reasoned about: with `loading.tsx` deleted from
`/reports`, `/settings`, `/contacts` and `/pipeline`, all four still flashed
(median 381ms) because Next falls back to the nearest ancestor boundary —
`src/app/(app)/loading.tsx`, which is the Today page's three-column agenda
shape. Those four files were restored. The same fallback means any route under
`(app)` without its own `loading.tsx` shows an agenda-shaped skeleton; that is
registered in `docs/KNOWN_GAPS.md`.

### Verification

231/231 on `npm test` (36 suites), `npm run lint` and `npx tsc --noEmit` clean.
Measured in a production build across 12 navigations before and after. The
shell constraint was re-checked explicitly after the change: the org name still
renders (`TEKGUYZ Demo`, proving the embedded organization select resolves
under RLS), and sidebar collapse is unchanged — the `tg_sidebar` cookie goes
`null` → `collapsed`, survives a navigation to `/contacts`, and returns to
`expanded` on a second toggle, with the rail's transition still on
`transform, translate, scale, rotate` and never a layout property. No database
rows were written by any of this.
