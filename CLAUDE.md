...
- **Genuinely closed, not just assumed:** Prompt 14's weekly report cron. `NEXT_PUBLIC_APP_URL` and `CRON_SECRET` are set (both `.env` and Vercel project env vars) and confirmed live in production (`/api/cron/weekly-report` correctly 401s on a missing/wrong secret). `send-weekly-report.ts`'s `APP_URL` now strips any trailing slash before use, fixing the double-slash the env var's own trailing slash was producing in the deep link — re-verified with two disposable test orgs (one WON/open, one zero-activity) run through the real cron logic locally: each report contained only its own org's numbers, went only to that org's own OWNER/ADMIN, the zero-activity org got the quiet-month narrative, and the deep link rendered with exactly one slash in all three sends (the two test orgs plus the real org). Test fixtures were deleted afterward, confirmed via SQL. **Residual, deliberately not touched:** `notify-new-lead.ts`'s `APP_URL` still has the same unstripped-trailing-slash pattern — only `send-weekly-report.ts` was asked to be fixed. Same one-line fix would apply there if it's ever worth doing.
