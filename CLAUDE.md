...
- **Design System v2 "Structural Neutral" (2 prompts).** ⬜ **Prompt 1 of 2 shipped 2026-08-14; Prompt 2 not started.** Prompt 1 replaced the Notion High-Voltage tokens wholesale (hue 60/240 → 260, Level 0 flat by default, tightened radius/type scales), swapped `lucide-react` → `@tabler/icons-react` across all 19 call sites, added Vitest + React Testing Library (49 tests, 8 suites — the repo had no test runner before), and built 8 primitives in `src/components/ui/` plus a dev-only `/design` kitchen-sink route. **Prompt 2 — rolling those primitives out across every existing view — is outstanding**, so views currently render in v2 tokens but still use one-off classes. One thing not done and easy to assume otherwise: Prompt 2 itself. `--accent` is no longer a placeholder — it was sampled and closed by the Brand Identity initiative below on 2026-08-14. Full history: `docs/ADDENDA_LOG.md` § Design System v2 "Structural Neutral" — foundation layer; open items in `docs/KNOWN_GAPS.md`.
- **Brand Identity "Converging Funnel" (1 prompt).** ✅ **Complete, shipped 2026-08-14.** Retired the TEKGUYZ agency mark from the CRM and replaced it with a purpose-built mark: an inverted funnel fed by three hexagonal nodes converging into one arrow. All assets are generated by `scripts/brand/build_brand.py` (10 SVGs, 11 PNGs, a multi-frame `.ico`) — never hand-edited. Wired into `layout.tsx` metadata, a new `src/app/manifest.ts` and a new `src/app/opengraph-image.tsx`; the three App Router icon-convention files under `src/app/` were deleted. This initiative also **closed the `--accent` placeholder** — the mark's blue was the reference the Design System v2 gap was waiting for. Two open items remain in `docs/KNOWN_GAPS.md`: nobody has eyeballed the mark against `docs/brand/PROOF-SHEET.png`, and the OG tagline is unapproved copy. Full history: `docs/ADDENDA_LOG.md` § Brand identity + `--accent` sampling.
