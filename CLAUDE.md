...
|---|---|---|---|---|---|
| Lead data | `next_action_at` (SLA date) | **NO edit UI** | Read in 8 places (`format.ts`, `pipeline.ts` sort, `getSlaCriticalLeads`); written only by DB default `now()+24h`, webhook, seed | `EditLeadModal` + `CreateLeadModal` | **P0** |
| Lead data | Archived lead recovery | **NO** | `archiveLead` sets `archived:true`; all 6 queries filter `archived=false`; only un-archive path in the codebase is the webhook Resurrection Engine | Contacts filter toggle + restore action | **P0** |
...
| Appearance | Theme toggle | **YES — built this pass** | `ThemeToggle.tsx` → `Header` | Header (correct — persistent chrome) | Done |
...
- **`leads.next_action_at` has no edit UI anywhere — P0, found in the 2026-07-25 settings/IA audit.** The "Going Cold" SLA rule is described in Section 1 of this file as a core mechanic, and `next_action_at` is read in 8 places (`isOverdue`/`formatDueAt` in `format.ts`, the sort in `lib/leads/pipeline.ts`, `getSlaCriticalLeads`, three card components) — but it is **written** only by the DB default (`now() + 24 hours`), the webhook ingest path, and the demo seed script. Confirmed by grep: it appears in neither `CreateLeadModal` nor `EditLeadModal` nor `lib/leads/actions.ts`'s insert/update payloads. Practical consequence: a user cannot reschedule a follow-up, so every lead is permanently pinned to `created_at + 24h` and the SLA Critical queue fills up with no way to clear it. **Disposition (2026-07-25): flagged, not fixed** — found during an explicitly audit-only pass whose constraint was "list, prioritize, stop." This is the single highest-priority functional gap in the app; fix before design work.
- **Archiving a lead is a one-way trip with no in-app recovery — P0, found in the 2026-07-25 audit.** `archiveLead` sets `archived: true` and all six read queries filter `.eq("archived", false)`; the only un-archive path in the entire codebase is the webhook Resurrection Engine (`ingest-lead.ts:59`), i.e. the customer re-submitting the public form themselves. There is no "view archived" filter and no restore action, and `EditLeadModal`'s "Archive lead" button submits immediately with no confirmation step. A mis-click silently removes a lead from every view permanently. **Disposition (2026-07-25): flagged, not fixed** (same audit-only constraint). Pair the fix with a confirmation step on the archive button.
