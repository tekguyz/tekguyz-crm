...
- **The sidebar's collapse state is the `tg_sidebar` cookie, read in the server layout — never `localStorage`.** localStorage cannot be read on the server or before hydration, so it paints expanded and snaps to the rail a frame later. Full shell spec: `docs/DESIGN.md` § The Application Shell.
...
| **Leads MEMBER-Role Enforcement** (1 prompt) | ✅ Complete, applied and proven live (15/15 on `npm run test:rls`) | 2026-08-14 | Leads MEMBER-role enforcement addendum |
