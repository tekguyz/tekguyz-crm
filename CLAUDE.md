...
- **Resolved as of the Prompt 14 verification pass:** `NEXT_PUBLIC_APP_URL` and `CRON_SECRET` are now set (both `.env` and Vercel project env vars), confirmed live in production (`/api/cron/weekly-report` correctly 401s on a missing/wrong secret). `NEXT_PUBLIC_APP_URL` currently has a trailing slash (`https://tekguyz-crm.vercel.app/`), which produces a cosmetic double-slash in deep links (`.../` + the code's own `/`) — browsers normalize this and it resolves fine, but if it's ever worth tidying, either strip the env var's trailing slash or have the two call sites (`notify-new-lead.ts`, `send-weekly-report.ts`) trim it defensively.
