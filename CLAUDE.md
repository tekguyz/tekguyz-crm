...
- **No password reset flow exists — and it's not "magic-link-only by design," it's genuinely absent.** Confirmed by reading `login/page.tsx` (plain email+password, no "forgot password" link anywhere) and grepping the whole codebase for `resetPasswordForEmail`/"forgot password"/"reset password" (zero matches). A user who forgets their password today has no self-service recovery path at all. **Disposition (2026-07-24): flagged, not fixed.** Found during the 2026-07-24 production gaps sweep's missing-pages audit; deliberately not built in that same pass — this needs its own scoped prompt (a "forgot password" link + page calling `supabase.auth.resetPasswordForEmail`, a reset-confirmation page, a new Supabase Auth email template, and a middleware allowlist entry for the new route), not a bolt-on. Revisit before this app has more than a single trusted user, since right now the only real account (`admin@tekguyz.com`) forgetting its password would require a direct database intervention to recover.
