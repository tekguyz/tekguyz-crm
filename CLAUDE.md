...
- **Design System v2 "Structural Neutral" (2 prompts).** ⬜ **Prompt 1 of 2 shipped 2026-08-14; Prompt 2 not started.** Prompt 1 replaced the Notion High-Voltage tokens wholesale (hue 60/240 → 260, Level 0 flat by default, tightened radius/type scales), swapped `lucide-react` → `@tabler/icons-react` across all 19 call sites, added Vitest + React Testing Library (49 tests, 8 suites — the repo had no test runner before), and built 8 primitives in `src/components/ui/` plus a dev-only `/design` kitchen-sink route. **Prompt 2 — rolling those primitives out across every existing view — is outstanding**, so views currently render in v2 tokens but still use one-off classes. Two things not done and easy to assume otherwise: `--accent` is still an unsampled placeholder, and nothing was verified by eye (the browser pane never composited; all visual checks were scripted computed-style reads). Full history: `docs/ADDENDA_LOG.md` § Design System v2 "Structural Neutral" — foundation layer; open items in `docs/KNOWN_GAPS.md`.
